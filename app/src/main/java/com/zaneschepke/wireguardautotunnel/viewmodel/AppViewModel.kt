...
constructor(
    ...
    init {
        viewModelScope.launch(ioDispatcher) {
            try {
                // Load server configurations first with timeout
                val configLoaded = withTimeout(10_000) { // 10 second timeout
                    configManager.loadConfigurations()
                }
                
                if (!configLoaded) {
                    Timber.w("Failed to load server configurations on startup")
                    handleShowMessage(StringValue.DynamicString("Failed to load server configurations"))
                }
                ...
                Timber.e("Timeout loading server configurations")
                handleShowMessage(StringValue.DynamicString("Server configuration loading timed out"))
            } catch (e: Exception) {
                Timber.e(e, "Error during app initialization")
                handleShowMessage(StringValue.DynamicString("App initialization error: ${e.localizedMessage}"))
            }
        }
    }
    ...
    private suspend fun handleConnectToTorusServer(
        ...
        try {
            ...
            val result = vpnConnectionManager.connectToServer(
                ...
                appSettings = appSettings,
                onShowMessage = { message -> handleShowMessage(message) },
                onRequestVpnPermission = { requestVpnPermission(true) }
            )
            
            when (result) {
                is VpnConnectionManager.ConnectionResult.Success -> {
                    Timber.i("Successfully connected to ${serverConfig.displayName}")
                    // Message is handled by the connection manager
                }
                
                is VpnConnectionManager.ConnectionResult.PermissionRequired -> {
                    Timber.i("Permission required for connection to ${serverConfig.displayName}")
                    // Permission request is handled by the connection manager
                }
                
                is VpnConnectionManager.ConnectionResult.Error -> {
                    Timber.e("Failed to connect to ${serverConfig.displayName}: ${result.message}")
    ...
    /**
     * Handles reloading server configurations from the JSON file
     */
    private suspend fun handleReloadServerConfigs() = withContext(ioDispatcher) {
        try {
            Timber.i("Reloading server configurations")
            val success = configManager.reloadConfigurations()
            
            if (success) {
                handleShowMessage(StringValue.DynamicString("Server configurations reloaded"))
            } else {
                handleShowMessage(StringValue.DynamicString("Failed to reload server configurations"))
            }
            
        } catch (e: Exception) {
            Timber.e(e, "Error reloading server configurations")
            handleShowMessage(StringValue.DynamicString("Error reloading configurations: ${e.message}"))
        }
    }
}
